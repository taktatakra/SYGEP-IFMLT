...
# ... (Le reste des fonctions de connexion et d'initialisation de la BDD reste identique) ...
...
if 'logged_in' not in st.session_state:
    ...
    st.session_state.current_menu = "Tableau de Bord" # Initialiser le menu par défaut
...
if st.sidebar.button("🚪 Se déconnecter", use_container_width=True):
    ...
    for key in list(st.session_state.keys()):
        if key not in ['logged_in', 'username', 'user_id', 'role', 'permissions', 'session_id', 'current_menu']: # Conserver les clés pour éviter les erreurs de state au rerun
            del st.session_state[key]
    st.rerun()

st.sidebar.divider()

# Définition du mapping du menu avec les Emojis
menu_mapping = {
    "Tableau de Bord": {"label": "📈 Tableau de Bord", "module": "tableau_bord"},
    "Gestion des Clients": {"label": "👥 Gestion des Clients", "module": "clients"},
    "Gestion des Produits": {"label": "📦 Gestion des Produits", "module": "produits"},
    "Gestion des Fournisseurs": {"label": "🚚 Gestion des Fournisseurs", "module": "fournisseurs"},
    "Gestion des Commandes": {"label": "🛒 Gestion des Commandes", "module": "commandes"},
    "Gestion des Achats": {"label": "🛍️ Gestion des Achats", "module": "achats"},
    "Rapports & Exports": {"label": "📊 Rapports & Exports", "module": "rapports"},
    "Gestion des Utilisateurs": {"label": "🔑 Gestion des Utilisateurs", "module": "utilisateurs"},
    "À Propos": {"label": "ℹ️ À Propos", "module": "about"}
}

# Filtrer les options de menu basées sur les permissions
menu_options_labels = []
menu_options_values = []
for value, data in menu_mapping.items():
    if has_access(data["module"]) or data["module"] == "about":
        menu_options_labels.append(data["label"])
        menu_options_values.append(value)

# Déterminer la valeur par défaut pour st.radio
default_index = 0
if 'current_menu' in st.session_state and st.session_state.current_menu in menu_options_values:
    default_index = menu_options_values.index(st.session_state.current_menu)

# Remplacer le st.selectbox par st.radio (bouton à cocher)
menu_label = "🧭 **Navigation Principale**"
menu_selection = st.sidebar.radio(
    menu_label, 
    options=menu_options_labels, 
    index=default_index, 
    key="main_menu_radio"
)

# Retrouver la valeur du menu sélectionné
menu = ""
for value, data in menu_mapping.items():
    if data["label"] == menu_selection:
        menu = value
        break

# Mettre à jour l'état de session pour persister la sélection
